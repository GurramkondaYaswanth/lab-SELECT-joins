...
--3.Write a query to return all names from the user_new table where there is matching user_new.role_id value in both the user_new and role tables.
    SELECT user_new.name, user_new.username
    FROM  USER_NEW
    INNER JOIN ROLE 
    ON user_new.role_id = role.id;
...
--profile id in both user_new and profile table,  profile.degree_id and degree.id from profile and degree table,  degree.department_id and department.id in degree and department
--profile.id and experience.profile_id in experience and profile table where user_name.name = "ProGrad" and experience.currentValue=3
...
--Display by user_new.name and skill.name
    ...
    inner join profile on user_new.profile_id = profile.id
    inner join skill on user_new.id = skill.id
    where role.name = 'Alumini';
...
--  - Display event in descending order.
    ...
    from user_new
    inner join event on user_new.role_id = event.id
    where role.name='Alumni'
